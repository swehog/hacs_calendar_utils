...
async def _handle_get_events(call: ServiceCall) -> dict:
    ...
    for calendar in calendars:
        events = [
            *events,
            *[
                {**x, "calendar_entity": calendar.entity_id}
                for x in await _async_get_events_service(calendar, call)
            ],
        ]
...
async def _async_get_events_service(
    ...
    start = service_call.data.get(
        EVENT_START_DATETIME, service_call.data.get(EVENT_START_DATE, dt_util.now())
    )

    if EVENT_DURATION in service_call.data:
        end = start + service_call.data[EVENT_DURATION]
    else:
        end = service_call.data.get(
            EVENT_END_DATETIME, service_call.data.get(EVENT_END_DATE, start)
        )
